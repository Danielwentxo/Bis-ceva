...
export const moderateImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: true } | { ok: false; reason: string }> => {
    ...
    if (!user || !secret) {
      return { ok: false, reason: "Image check is not configured yet." };
    }
    ...
    body.set("media", new Blob([bytes], { type: "image/jpeg" }), "logo.jpg");
    ...
    if (!response.ok || !json || json.status === "failure") {
      return { ok: false, reason: json?.error?.message || "Could not check this image. Try another photo." };
    }
