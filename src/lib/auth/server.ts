...
import { sendResetPasswordEmail } from "./reset-email.server";
import {
  GROK_ISSUER_DEFAULT,
...
// Direct Google / Apple sign-in (bypasses the Grok broker entirely — this app
// holds its own OAuth credentials). Each is only wired up when its env vars
// are present, so leaving them unset is a safe no-op (no broken buttons).
//
// Google: console.cloud.google.com -> APIs & Services -> Credentials ->
//   "OAuth client ID" (type: Web application). Authorized redirect URI:
//   `${BETTER_AUTH_URL}/api/auth/callback/google` (or the localhost/preview
//   equivalent while testing). Free.
const googleClientId = env("GOOGLE_CLIENT_ID");
const googleClientSecret = env("GOOGLE_CLIENT_SECRET");

// Apple: developer.apple.com (requires a **paid** $99/yr Apple Developer
// account) -> Certificates, Identifiers & Profiles -> create a Services ID
// (this is `APPLE_CLIENT_ID`) with "Sign in with Apple" enabled, redirect URI
// `${BETTER_AUTH_URL}/api/auth/callback/apple`. `APPLE_CLIENT_SECRET` is a
// short-lived JWT you (re)generate per Apple's docs — Better Auth does not
// generate it for you.
...
export const auth = betterAuth({
  ...
  ...(emailAndPasswordEnabled
    ? {
        emailAndPassword: {
          ...
          // "Am uitat parola" flow: Better Auth generates + verifies the
          // token; we're only responsible for emailing the link. See
          // `./reset-email.server` — without RESEND_API_KEY set this just
          // logs the link to the server console instead of sending it.
  ...
  // Direct Google / Apple sign-in, only when credentials are configured (see
  // above). Independent of the Grok broker's `genericOAuth` providers below —
  // both can be active at once; the sign-in UI just shows whichever exist.
