...
/**
 * Resolve a local binary from `node_modules/.bin` (Windows-safe).
 * On Windows the shim is `vite.cmd`; without PATH + shell, spawn('vite') fails
 * with ENOENT even after a successful npm install.
 */
function resolveLocalBin(root, command) {
  const binDir = join(root, "node_modules", ".bin");
  if (process.platform === "win32") {
    for (const name of [`${command}.cmd`, `${command}.ps1`, `${command}.exe`, command]) {
...
function main(argv) {
  ...
  // Ensure node_modules/.bin is first on PATH (mirrors what npm scripts do).
  const binDir = join(root, "node_modules", ".bin");
  const pathKey = process.platform === "win32" ? "Path" : "PATH";
  const existingPath = env[pathKey] || env.PATH || process.env.PATH || "";
  env[pathKey] = `${binDir}${process.platform === "win32" ? ";" : ":"}${existingPath}`;
  if (pathKey !== "PATH") env.PATH = env[pathKey];

  const resolved = resolveLocalBin(root, command);
  ...
  child.on("error", (err) => {
    ...
    console.error(
      `[with-app-env] tip: rulează "npm install" în folderul proiectului, apoi reîncearcă.`,
    );
    process.exit(127);
  });
